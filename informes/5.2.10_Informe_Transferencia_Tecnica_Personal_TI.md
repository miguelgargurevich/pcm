# 5.2.10 Informe de Transferencia Técnica

**INFORME TÉCNICO**

**ASUNTO:** Informe de Transferencia Técnica: Infraestructura de Datos, Despliegue y Funcionamiento de Módulos – Plataforma de Cumplimiento Digital (PCD)

**FECHA:** 30 de diciembre de 2025

**DIRIGIDO A:** Personal de Tecnologías de la Información - SGTD-PCM
...
El presente documento detalla las actividades de transferencia técnica correspondientes al **hito 5.2.10** de los Términos de Referencia. Se describe el proceso de configuración, despliegue y puesta a punto de la arquitectura de persistencia de datos y lógica de negocio en los servidores de producción de la PCM.

### 1.1 Objetivos del Documento

- Proporcionar un **manual de instalación** completo para el personal técnico de TI
- Documentar la **configuración del archivo `appsettings.json`** y variables de entorno
- Detallar la **cadena de conexión** a la base de datos
- Registrar las dificultades encontradas durante el proceso de despliegue
- Transferir el conocimiento técnico necesario para la operación y mantenimiento del sistema

### 1.2 Material Complementario

> 📹 **Video de Transferencia Técnica:** Se adjunta un video explicativo de aproximadamente 5 minutos donde se demuestra el proceso de despliegue de la plataforma, incluyendo la configuración de servicios y verificación de funcionamiento.

---

## 2. DIAGNÓSTICO DE INFRAESTRUCTURA Y ADECUACIÓN TECNOLÓGICA

### 2.1 Discrepancia de Ambientes

Es fundamental precisar que, durante la fase de despliegue, el equipo consultor identificó una **discrepancia crítica** entre los ambientes proyectados y los ambientes finales asignados:
...
### 2.2 Protocolo de Adecuación Ejecutado

Ante este escenario, se ejecutó un **protocolo de adecuación de infraestructura no contemplado originalmente**, que incluyó:

| Actividad | Descripción | Estado |
|-----------|-------------|--------|
| **Seguridad Perimetral** | Configuración de firewall UFW a nivel de Sistema Operativo | ✅ Completado |
| **Gestión de Acceso** | Configuración de permisos y llaves SSH para acceso seguro | ✅ Completado |
| **Dependencias** | Instalación manual de .NET 9 SDK, Node.js 20 LTS, Nginx | ✅ Completado |
| **Optimización Kernel** | Ajustes para gestión óptima de contenedores y servicios | ✅ Completado |

---

## 3. TRANSFERENCIA TÉCNICA: CAPA DE PERSISTENCIA (DATABASE)

El componente central de la PCD reside en un motor de base de datos relacional robusto, configurado bajo estándares de alta disponibilidad y seguridad institucional.
...
| **Compilación** | x86_64-pc-linux-gnu |
| **Host de Producción** | `101.44.10.71` |
...
> ⚠️ **NOTA DE SEGURIDAD:** La contraseña real debe ser proporcionada por canales seguros y no debe ser almacenada en documentación pública.

### 3.3 Proceso de Hardening y Configuración de Acceso

Se han realizado ajustes críticos en los archivos de configuración del motor para garantizar la **interoperabilidad a nivel nacional** solicitada en el TDR:

#### Archivo `postgresql.conf`
```conf
# Permitir conexiones desde endpoints externos (después de firewall)
listen_addresses = '*'
...
# Autenticación IPv4 (con contraseña cifrada)
...
### 3.4 Estructura de Datos (Schema) y Migración

Se completó la migración e importación de la estructura de datos, compuesta por **61 tablas relacionales** que soportan los siguientes módulos:

| # | Módulo | Tablas Principales |
|---|--------|-------------------|
| 1 | **Gestión de Usuarios** | `usuarios`, `perfiles`, `perfil_permisos`, `permisos_modulos` |
| 2 | **Gestión de Entidades** | `entidades`, `sectores`, `niveles_gobierno`, `ubigeos` |
| 3 | **Marco Regulatorio** | `marcos_normativos`, `tipos_norma` |
| 4 | **Gestión de Compromisos** | `compromisos_gobierno_digital`, `compromiso_normativas`, `alcances_compromisos` |
| 5 | **Cumplimiento Normativo** | `cumplimientos_normativos`, `cumplimiento_historial`, `criterios_evaluacion` |
| 6 | **Evaluación & Cumplimiento** | `evaluacion_respuestas_entidad`, `ceda_evaluacion_cab`, `ceda_evaluacion_det` |
| 7 | **Compromisos Específicos** | `com1_lider_gtd` hasta `com21_oficial_gobierno_datos` |

El proceso de migración se realizó mediante:
```bash
...
### 3.5 Métodos de Administración Transferidos

Se han validado **dos canales de acceso** para el equipo técnico de la PCM:

#### Opción 1: Administración vía SSH Tunnel (Recomendado)
```bash
# Crear túnel SSH seguro
...
#### Opción 2: Conexión Directa con Cliente gráfico
- **DBeaver**: Configurar nueva conexión PostgreSQL con los parámetros indicados
...
|----------|---------|-----------|
| **Docker** | 24.0+ | Contenedorización del Backend |
...
> ⚠️ **NOTA:** Después de agregar el usuario al grupo docker, debe cerrar sesión y volver a iniciar para que los cambios surtan efecto.
...
## 5. CONFIGURACIÓN DEL ARCHIVO `appsettings.json`
...
```json
{
  ...
  "ConnectionStrings": {
    "DefaultConnection": "Host=101.44.10.71;Port=5432;Database=pcd_plataforma;Username=postgres;Password=[CONTRASEÑA_SEGURA]"
  },
...
### 5.3 Descripción de Parámetros Críticos

| Sección | Parámetro | Descripción | Obligatorio |
|---------|-----------|-------------|-------------|
| **ConnectionStrings** | `DefaultConnection` | Cadena de conexión a PostgreSQL | ✅ Sí |
| **JwtSettings** | `SecretKey` | Clave para firmar tokens JWT (mín. 32 chars) | ✅ Sí |
| **JwtSettings** | `ExpirationMinutes` | Tiempo de vida del token en minutos | ✅ Sí |
| **ReCaptcha** | `SecretKey` | Clave secreta de Google reCAPTCHA v3 | ⚠️ Recomendado |
| **Aws** | `AccessKeyId`, `SecretAccessKey` | Credenciales AWS SES para envío de correos | ✅ Configurado |
| **Aws** | `SesFromEmail` | Email verificado en AWS SES | ✅ Configurado |
| **FileStorage** | `BasePath` | Ruta local para almacenamiento de archivos | ✅ Sí |
| **Cors** | `Origins` | Dominios permitidos para CORS | ✅ Sí |

### 5.4 Configuración Alternativa: Variables de Entorno

En lugar de modificar `appsettings.json`, se pueden usar **variables de entorno** (recomendado para producción):
...
ConnectionStrings__DefaultConnection=Host=101.44.10.71;Port=5432;Database=pcd_plataforma;Username=postgres;Password=[CONTRASEÑA]
...
- **Almacenamiento Local** mediante volúmenes Docker

### 6.2 Dockerfile del Backend

El archivo `Dockerfile.local` (para producción) incluye compilación multi-etapa optimizada:
...
El archivo `frontend/Dockerfile` incluye compilación de React y servicio con Nginx:
...
#### A. Configuración Local (Pruebas - `docker-compose.local.yml`)
...
#### B. Configuración Servidor Linux x86_64 (`docker-compose.server.yml`)
...
- ✅ Construcción de imágenes Docker para arquitectura amd64
- ✅ Levantamiento de contenedores
- ✅ Verificación de health checks
- ✅ Despliegue de ambos servicios (backend y frontend)
...
### 6.6 Documentación Complementaria Creada

Se han creado los siguientes documentos técnicos en el repositorio:

| Documento | Ubicación | Descripción |
|-----------|-----------|-------------|
| **DESPLIEGUE_SERVIDOR_CLIENTE.md** | `/` | Guía completa de despliegue paso a paso con troubleshooting |
| **TRANSFERENCIA_SERVIDOR.md** | `/` | Opciones para transferir el proyecto al servidor (SCP, Git, rsync) |
| **FRONTEND_SERVIDOR.md** | `/` | Configuración específica del frontend y verificaciones |
| **DOCKER_LOCAL_README.md** | `/docs/` | Guía para pruebas locales con Docker |
| **start-server-docker.sh** | `/` | Script automatizado de despliegue |

### 6.7 Pruebas de Despliegue Realizadas

#### Prueba 1: Construcción Multi-Arquitectura
...
- **Imágenes**: Construidas correctamente desde máquina ARM64 (Mac M1/M2)
...
- Build ARG variables aplicadas correctamente durante la compilación
- Nginx sirviendo aplicación en puerto 3000 → 80
...
- Directorio `/app/storage/documentos` creado automáticamente

### 6.8 Credenciales de Acceso para Pruebas

| Perfil | Email | Contraseña | Descripción |
|--------|-------|------------|-------------|
...
| **Operador PCM** | operador.test@pcm.gob.pe | Admin123! | Seguimiento y evaluación |

> 📚 **Nota:** Ver archivo `docs/USUARIOS_PRUEBA.md` para la lista completa de usuarios de prueba
...
if nc -zv $DB_HOST $DB_PORT 2>&1 | grep -q "succeeded\|open"; then
    echo "✅ Conexión a PostgreSQL disponible"
else
...
services:
  pcm-backend:
    ...
    environment:
      ...
      - Smtp__Password=[CONTRASEÑA_SMTP]
...
├── storage/                     # Volumen: Archivos subidos (PDFs)
│   ├── evidencias/
│   └── documentos/
├── logs/                        # Volumen: Logs de la aplicación
│   ├── backend-*.log
│   └── nginx-*.log
└── frontend/
    └── dist/                    # React build (archivos estáticos)
```
...
### 6.7 Activar Configuración Nginx
...
## 7. FUNCIONAMIENTO DE LOS MÓDULOS (LÓGICA DE NEGOCIO)

### 7.1 Arquitectura de Servicios con Docker

El despliegue de los módulos sigue una **arquitectura orientada a contenedores**:
...
│                         NGINX (Puerto 80/443)                    │
│  ┌─────────────────────────┐  ┌─────────────────────────────┐   │
│  │  Frontend React (/)     │  │  Reverse Proxy (/api)       │   │
│  │  Archivos estáticos     │  │  → Docker Container :5000   │   │
│  └─────────────────────────┘  └─────────────────────────────┘   │
└───────────────────────────┬─────────────────────────────────────┘
                            │
                            ▼
┌─────────────────────────────────────────────────────────────────┐
│              DOCKER CONTAINER: pcm-backend                       │
│  ┌─────────────────────────────────────────────────────────┐    │
│  │  .NET 9 Runtime + PCM.API.dll                           │    │
│  │  Controllers → Services → Handlers → DbContext          │    │
│  │  (MediatR CQRS Pattern)                                 │    │
│  └─────────────────────────────────────────────────────────┘    │
│  ┌─────────────────────────────────────────────────────────┐    │
│  │  Volúmenes Montados:                                    │    │
│  │  • /var/www/pcm/storage → /app/storage (PDFs)           │    │
│  │  • /var/www/pcm/logs → /app/logs                        │    │
│  └─────────────────────────────────────────────────────────┘    │
└───────────────────────────┬─────────────────────────────────────┘
                            │ Npgsql Driver
...
│               AWS SES (Amazon Simple Email Service)              │
│               Region: us-east-1                                  │
│               Dominio: plataformacumplimientodigital.servicios.gob.pe │
...
| **Backend Runtime** | .NET 9.0 | Entorno de ejecución de la API |
| **ORM** | Entity Framework Core 9.0 | Mapeo objeto-relacional |
| **Driver PostgreSQL** | Npgsql 9.0.4 | Comunicación con la base de datos |
| **Autenticación** | JWT Bearer | Tokens de acceso seguros |
| **Validación** | FluentValidation | Validación de datos de entrada |
| **Documentación API** | Swagger/OpenAPI | Documentación interactiva |

### 7.3 Seguridad de Capa

- **Autenticación JWT**: Cada request a endpoints protegidos requiere token válido
- **Autorización RBAC**: Control de acceso basado en roles (Admin, Operador, Entidad)
- **Validación de Entrada**: FluentValidation previene inyección de datos maliciosos
...
### 8.1 Dificultades Técnicas Resueltas

| # | Dificultad | Impacto | Solución Implementada |
|---|------------|---------|----------------------|
| 1 | **Cambio de SO** (Windows → Linux) | Alto | Reconfiguración completa de scripts de despliegue y uso de Docker |
| 2 | **Configuración SSH** sin documentación previa | Medio | Generación de llaves y configuración de acceso |
| 3 | **Permisos de firewall** no documentados | Medio | Apertura de puertos 80, 443, 5432 en UFW |
| 4 | **Falta de certificado SSL** | Medio | Pendiente - se configuró HTTP temporal |
| 5 | **Adaptación a Docker** | Medio | Creación de Dockerfile y docker-compose para producción |

### 8.2 Dependencias Pendientes de la SGTD-PCM

Para la **culminación del despliegue final** y funcionamiento completo de la plataforma, se requiere que la SGTD-PCM provea los siguientes parámetros técnicos:

| # | Requerimiento | Descripción | Formato Esperado | Estado | Responsable |
|---|---------------|-------------|------------------|--------|-------------|
| 1 | **AWS SES Email Service** | Servicio de correo AWS SES configurado | Credenciales AWS + Dominio verificado | ✅ Completado | PCM |
| 2 | **Certificado SSL** | Certificado para habilitar HTTPS en producción | Archivos `.crt` y `.key` | 🔴 Pendiente | SGTD |
| 3 | **Dominio de Producción** | DNS institucional para la plataforma | `cumplimiento.pcm.gob.pe` | 🔴 Pendiente | SGTD |
| 4 | **Credenciales de Repositorio** | Acceso al repositorio institucional de código fuente | URL Git + token/credenciales | 🔴 Pendiente | SGTD |

> **NOTA:** El almacenamiento de archivos (PDFs, evidencias) se realizará localmente en el servidor host mediante volúmenes Docker, por lo que solo se requiere definir la ruta base y asegurar permisos de escritura. La ruta predeterminada configurada es `/var/www/pcm/storage`.

### 8.3 Configuración de AWS SES Implementada

El servicio de correo electrónico ha sido configurado usando **AWS SES (Amazon Simple Email Service)** con las siguientes credenciales:
...
|------------------------|----------------------|------------|--------|
| Notificaciones por email | AWS SES | 🔴 Alta | ✅ Completado |
| Recuperación de contraseña | AWS SES | 🔴 Alta | ✅ Completado |
| Conexión segura HTTPS | Certificado SSL | 🟡 Media | 🔴 Pendiente |
| URL de producción final | Dominio institucional | 🟡 Media | 🔴 Pendiente |

---

## 9. COMANDOS ÚTILES DE ADMINISTRACIÓN

### 9.1 Gestión del Contenedor Docker
...
# Recargar Nginx para servir nuevos archivos estáticos
sudo systemctl reload nginx
...
### 9.5 Gestión de Archivos Subidos (Storage Local)
...
| 6 | Frontend carga | Acceder desde navegador | Página de login |
| 7 | Conexión a BD | `docker logs pcm-backend` | Sin errores de conexión |
...
|------------|--------|-------------|
| **Base de Datos** | ✅ 100% Operativa | PostgreSQL configurado y accesible |
| **Backend API** | ✅ 100% Operativo | .NET 9 desplegado como servicio |
| **Frontend** | ✅ 100% Operativo | React build desplegado en Nginx |
| **Autenticación** | ✅ Funcional | JWT configurado |
| **Notificaciones Email** | ✅ Operativo | AWS SES configurado con dominio verificado |
| **HTTPS** | ⚠️ Pendiente | Requiere certificado SSL de SGTD |
| **Storage de PDFs** | ✅ Configurado | Almacenamiento local en `/var/www/pcm/storage` |

### 11.2 Resumen Ejecutivo

La **Plataforma de Cumplimiento Digital** se encuentra operativa en su núcleo funcional:

1. ✅ La **base de datos PostgreSQL** está 100% operativa y accesible, cumpliendo con los requisitos de interoperabilidad establecidos en el TDR.

2. ✅ El **backend .NET 9 en Docker** está desplegado y funcionando como contenedor, con reinicio automático configurado y healthcheck habilitado.

3. ✅ El **frontend React** está compilado y sirviendo a través de Nginx con configuración optimizada.

4. ✅ El **almacenamiento de archivos** está configurado localmente en el host mediante volúmenes Docker.

5. ✅ El **servicio de email AWS SES** está configurado con dominio verificado `plataformacumplimientodigital.servicios.gob.pe`.

6. ⚠️ Quedan **pendientes por parte de la SGTD**: certificado SSL y dominio de producción.

La transferencia técnica garantiza que el **personal de TI de la PCM** pueda:
- Gestionar contenedores Docker (iniciar, detener, actualizar)
- Realizar actualizaciones de código mediante `git pull` y rebuild
- Escalar la solución según sea necesario
...
├── storage/                     # Volumen: Archivos subidos (PDFs, evidencias)
│   ├── evidencias/
│   └── documentos/
├── logs/                        # Volumen: Logs de aplicación
│   ├── nginx-access.log
│   └── nginx-error.log
├── frontend/
│   └── dist/                    # React build (archivos estáticos)
...
| 80 | Nginx (HTTP) | TCP | Público |
| 443 | Nginx (HTTPS) | TCP | Público (pendiente SSL) |
...
| Contenedor | Imagen Base | Puerto Interno | Volúmenes |
|------------|-------------|----------------|-----------|
| `pcm-backend` | `mcr.microsoft.com/dotnet/aspnet:9.0` | 5000 | `/app/storage`, `/app/logs` |

### Anexo D: Comandos Docker Rápidos
...
> 📌 **RECORDATORIO:** Este documento debe ser tratado como **información sensible** ya que contiene detalles de configuración de infraestructura. Se recomienda su almacenamiento en ubicaciones con acceso controlado.
