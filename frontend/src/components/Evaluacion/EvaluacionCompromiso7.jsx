...
import { FileText, Globe, User, Mail, Phone, Calendar } from 'lucide-react';
import CriteriosEvaluacionList from './CriteriosEvaluacionList';

/**
 * Compromiso 7: Implementación de Mesa de Partes Digital
 */
const EvaluacionCompromiso7 = ({ data, activeTab, criterios = [], onVerDocumento }) => {
  if (!data) {
    return (
  ...
  const d = {
    urlMpd: data.urlMpd || data.UrlMpd || '',
    fechaImplementacion: data.fechaImplementacionMpd || data.FechaImplementacionMpd || null,
    responsable: data.responsableMpd || data.ResponsableMpd || '',
    correo: data.correoResponsableMpd || data.CorreoResponsableMpd || '',
    telefono: data.telefonoResponsableMpd || data.TelefonoResponsableMpd || '',
    tipoMpd: data.tipoMpd || data.TipoMpd || '',
    observacion: data.observacionMpd || data.ObservacionMpd || '',
  ...
  if (data.urlMpd) {
    documentos.push({
      id: 3,
      nombre: 'URL Mesa de Partes Digital',
      url: data.urlMpd,
      tipo: 'url'
    });
  }
    ...
    <div className="space-y-3">
      {activeTab === 'general' && (
        <div className="space-y-6">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-500 mb-1">URL Mesa de Partes Digital</label>
            <div className="bg-gray-50 rounded-md p-3 flex items-center gap-2">
              <Globe size={18} className="text-blue-600" />
              {d.urlMpd ? (
                <a href={d.urlMpd} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">
                  {d.urlMpd}
                </a>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div>
              <label className="block text-sm font-medium text-gray-500 mb-1">Tipo de Mesa de Partes</label>
              <div className="bg-gray-50 rounded-md p-3 text-sm text-gray-900">{d.tipoMpd || '-'}</div>
            </div>
            <div>
              ...
              <div className="bg-gray-50 rounded-md p-3 text-sm text-gray-900">
                {d.fechaImplementacion ? new Date(d.fechaImplementacion).toLocaleDateString('es-PE') : '-'}
              </div>
            </div>
          </div>
...
EvaluacionCompromiso7.propTypes = {
...
export default EvaluacionCompromiso7;
