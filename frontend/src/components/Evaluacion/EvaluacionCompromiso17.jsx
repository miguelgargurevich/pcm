 ...
 */
const EvaluacionCompromiso17 = ({ data, activeTab, criterios = [], onVerDocumento }) => {
  if (!data) {
    return (
        ...
        <div className="space-y-6">
          ...
          <div className="bg-cyan-50 rounded-lg p-4 border border-cyan-200">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div>
                <label className="text-sm text-gray-500 flex items-center gap-1"><Activity size={12} /> Estado del Plan</label>
                <div className={`mt-1 inline-block px-3 py-1 rounded-full text-sm font-medium ${getEstadoColor(d.estadoPlan)}`}>
                  {d.estadoPlan || '-'}
                </div>
              </div>
              <div>
                <label className="text-sm text-gray-500 flex items-center gap-1"><Calendar size={12} /> Fecha de Formulación</label>
