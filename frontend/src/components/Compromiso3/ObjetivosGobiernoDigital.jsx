...
/**
 * Componente para gestionar Objetivos de Gobierno Digital
 * @param {Array} objetivos - Lista de objetivos de gobierno digital
 * @param {Function} onObjetivosChange - Callback cuando cambian los objetivos
 * @param {boolean} viewMode - Modo solo lectura
 */
const ObjetivosGobiernoDigital = ({ objetivos = [], onObjetivosChange, viewMode = false }) => {
  ...
  const generarCodigoObjetivo = () => {
    const count = localObjetivos.filter(o => o.tipoObj === 'G').length + 1;
    return `OGD-${count.toString().padStart(2, '0')}`;
  };
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="space-y-3">
        {objetivosGD.length === 0 ? (
          <p className="text-gray-500 text-center py-4">No hay objetivos de gobierno digital registrados</p>
        ) : (
          objetivosGD.map((objetivo) => {
              ...
              <div key={objetivoId} className="bg-white border border-gray-200 rounded-lg overflow-hidden">
                ...
                <div className="flex items-center justify-between p-4 bg-green-50">
                  <div className="flex items-center gap-3 flex-1">
                    ...
                    <span className="font-mono text-green-600 font-semibold">{objetivo.numeracionObj}</span>
                        ...
                        return (
                          <div key={accionId} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                            <div className="flex items-center gap-3">
                              <span className="font-mono text-teal-600 font-medium">{accion.numeracionAcc}</span>
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg max-h-[90vh] flex flex-col">
            ...
            <div className="flex items-center justify-between p-6 border-b bg-gradient-to-r from-primary-600 to-primary-700 rounded-t-xl text-white">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-lg font-semibold">{editingObjetivo ? 'Editar Objetivo GD' : 'Nuevo Objetivo GD'}</h2>
                  <p className="text-sm text-white/80">Objetivos de gobierno digital</p>
                </div>
              </div>
              <button
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md max-h-[90vh] flex flex-col">
            ...
            <div className="flex items-center justify-between p-6 border-b bg-gradient-to-r from-primary-600 to-primary-700 rounded-t-xl text-white">
              <div className="flex items-center gap-3">
                <Plus className="w-6 h-6" />
                <div>
                  <h2 className="text-lg font-semibold">{editingAccion ? 'Editar Acción' : 'Nueva Acción'}</h2>
                  <p className="text-sm text-white/80">Acciones del objetivo</p>
                </div>
              </div>
              <button
...
export default ObjetivosGobiernoDigital;
